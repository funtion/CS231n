...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (eps,gamma,beta, mu,xmu,sq,var,std,istd,xhat,gammax,out)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  eps,gamma,beta, mu,xmu,sq,var,std,istd,xhat,gammax,out = cache
  N,D = dout.shape
  ...
  dstd =  -1.0/(std * std ) * distd

  dvar = 0.5/ np.sqrt(var + eps) * dstd
  
  dsq = 1.0/N * np.ones_like(dout) * dvar
  ...
  dx += 1.0/N * np.ones_like(dout) * dmu
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(N):
    for j in range(C):
      for k in range(Hp):
        for l in range(Wp):
          mx = x[i, j, k * stride:k * stride+pool_height, l * stride:l* stride+pool_width].max()
          dx[i, j, k * stride:k * stride+pool_height, l * stride:l* stride+pool_width] += dout[i,j,k,l] * (x[i, j, k * stride:k * stride+pool_height, l * stride:l* stride+pool_width] == mx)
