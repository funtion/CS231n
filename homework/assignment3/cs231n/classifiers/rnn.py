...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    embedded_words_in, cache_in = word_embedding_forward(captions_in, W_embed)
    if self.cell_type == "rnn":
        hidden_states, cache_rnn = rnn_forward(embedded_words_in, h0, Wx, Wh, b)
        ...
        grads["W_embed"] = word_embedding_backward(dout, cache_in)
        grads["W_proj"] = features.T.dot(dh0)
        grads["b_proj"] = dh0.sum(axis=0)
    else:
        hidden_states, cache_lstm = lstm_forward(embedded_words_in, h0, Wx, Wh, b)
        scores, cache_affine = temporal_affine_forward(hidden_states, W_vocab, b_vocab)
  ...
  def sample(self, features, max_length=30):
    ...
    for i in range(max_length):
        if self.cell_type == "rnn":
            ...
            captions[:, i] = np.argmax(next_h.dot(W_vocab) + b_vocab, axis = 1)
            prev_h = next_h
            x = np.array([W_embed[v] for v in captions[:, i]])
        else:
